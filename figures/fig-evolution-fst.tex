...
\tikzstyle{e}=[lab,midway,auto=false,fill=TolHighContrastBlue,font=\scriptsize]
...
\draw[line] (M_sub) to[bend right=22.5] node[e,pos=0.5]
{X X X\textrightarrow{}Y Y Y} (S_sub);
...
\node[above right=-1.80cm and 5mm of sequences,text width=50mm] {
\begin{tabular}{r@{\,: }l}
...
\draw[line] (I1) to[bend right=30] node[e] {\O\textrightarrow{}Z} node[w,below=2mm] {$\pi_{\text{Z}}$} (U1);
...
\draw[line] (M1) to[bend left=45] node[e,pos=0.25] {Y\textrightarrow{}Z} (S1);
