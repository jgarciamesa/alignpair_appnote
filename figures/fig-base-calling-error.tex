...
\draw[line] (ij_sub) to[bend right=20] node[p] {$\varepsilon$} (S_sub);
\draw[line] (M_sub) to node[e] {i\textrightarrow{}i} (ii_sub);
\draw[line] (ii_sub) to node[p] {$1 - 3 \cdot \varepsilon$} (S_sub);
...
\multicolumn{2}{l}{\textbf{Sequences}}\\
    \hline
	i & input nucleotides\\
	j & intermediate nucleotides\\
	N & input ambiguous nucleotide N\\
\end{tabular}
};

\node[above right=-1.03cm and 5mm of sequences,text width=50mm] {
\begin{tabular}{r@{\,: }l}
