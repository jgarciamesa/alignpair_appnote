...
methods <- c("clustalo", "macse", "mafft", "prank", "coati-tri-mg")
aln_data2 <- aln_data |> filter(method %in% methods)

gg1 <- ggplot(aln_data2, aes(x = k2p, color=method)) + geom_density(linewidth = 1)
...
gg2 <- ggplot(k2p_data, aes(x = k2p_norm, color=method)) + geom_density(linewidth = 1, bw=0.07)
