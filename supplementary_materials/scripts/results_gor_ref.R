...
gor_ref_results = function() {
    ...
    results_raw = read.csv("data/gor-ref/results/results_summary.csv",
                       header = TRUE)
    alns = results_raw |> filter(aligner == "gor-ref") |> drop_na(score) |> select(ref_name) |> unlist()
    results = results_raw |> filter(ref_name %in% alns)
    results_file = "data/gor-ref/results/results_summary_clean.csv"
...
filter_counts = function() {
    results = read.csv("gor-ref/results/results_summary.csv", header = TRUE)
    ...
    for(aln_name in filtered) {
        aln = read.fasta(file = paste0("tri-mg/data/no_gaps_ref/",
                                       aln_name),
                         seqonly = TRUE)
